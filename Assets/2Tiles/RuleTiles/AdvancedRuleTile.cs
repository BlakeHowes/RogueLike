...
[CreateAssetMenu(fileName = "Advanced Rule Tile", menuName = "2D/Tiles/Advanced Rule Tile")]
public class AdvancedRuleTile : RuleTile<AdvancedRuleTile.Neighbor> {
    ...
    public void FillRules() {
        foreach(var rule in m_TilingRules) {
            foreach(var neighbour in rule.GetNeighbors()) {
                //if(neighbour.Value == 1) { neighbour.Value = 4; }
            }
        }
    ...
    public override bool RuleMatch(int neighbor, TileBase tile) {
        switch (neighbor) {
            case Neighbor.This: return Check_Specified(tile);
